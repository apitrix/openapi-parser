...
func TestXML_SetName_WithHook_Passes(t *testing.T) {
	x := NewXML("old", "", "", false, false)
	x.Trix.OnSet("name", func(field string, oldVal, newVal interface{}) error {
		return nil
	})
...
func TestXML_SetAttribute_WithHook_Rejects(t *testing.T) {
	x := NewXML("", "", "", false, false)
	rejectErr := errors.New("rejected")
	x.Trix.OnSet("attribute", func(field string, oldVal, newVal interface{}) error {
		return rejectErr
	})
	err := x.SetAttribute(true)
	if err != rejectErr {
		t.Errorf("SetAttribute with rejecting hook should return error, got %v", err)
	}
	if x.Attribute() {
		t.Errorf("Attribute should be unchanged after rejection")
	}
}
