...
func TestPaths_SetItems_WithoutHook(t *testing.T) {
	p := NewPaths(nil)
	items := map[string]*PathItem{"/pets": NewPathItem("", nil, nil, nil, nil, nil, nil, nil, nil)}
...
func TestPaths_SetItems_WithHook_Passes(t *testing.T) {
	p := NewPaths(nil)
	p.Trix.OnSet("items", func(field string, oldVal, newVal interface{}) error {
		return nil
	})
	...
	err := p.SetItems(items)
	if err != nil {
		t.Fatalf("SetItems with passing hook should succeed, got %v", err)
	}
	if p.Items()["/pets"] != items["/pets"] {
		t.Errorf("Items() = %v, want %v", p.Items(), items)
	}
