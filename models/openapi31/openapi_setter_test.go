...
func TestOpenAPI_SetOpenAPIVersion_WithoutHook(t *testing.T) {
	o := NewOpenAPI("3.1.0", NewInfo("API", "", "", "", "1.0", nil, nil))
	o.openAPI = "old"
...
func TestOpenAPI_SetJsonSchemaDialect_WithoutHook(t *testing.T) {
	o := NewOpenAPI("3.1.0", NewInfo("API", "", "", "", "1.0", nil, nil))
	err := o.SetJsonSchemaDialect("https://json-schema.org/draft/2020-12/schema")
	if err != nil {
		t.Fatalf("SetJsonSchemaDialect without hook should succeed, got %v", err)
	}
	if o.JsonSchemaDialect() != "https://json-schema.org/draft/2020-12/schema" {
		t.Errorf("JsonSchemaDialect() = %q, want draft/2020-12", o.JsonSchemaDialect())
	}
}
