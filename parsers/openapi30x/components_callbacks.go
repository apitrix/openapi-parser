...
func parseComponentsCallbacks(parent *yaml.Node, ctx *ParseContext) (map[string]*shared.Ref[openapi30models.Callback], error) {
	node := nodeGetValue(parent, "callbacks")
	if node == nil || !nodeIsMapping(node) {
		return nil, nil
	}

	callbacks := make(map[string]*shared.Ref[openapi30models.Callback])
