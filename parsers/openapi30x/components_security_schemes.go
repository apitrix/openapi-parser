...
func parseComponentsSecuritySchemes(parent *yaml.Node, ctx *ParseContext) (map[string]*openapi30models.RefSecurityScheme, error) {
	node := nodeGetValue(parent, "securitySchemes")
	if node == nil || !nodeIsMapping(node) {
		return nil, nil
	}

	schemes := make(map[string]*openapi30models.RefSecurityScheme)
