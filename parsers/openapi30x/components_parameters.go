...
func parseComponentsParameters(parent *yaml.Node, ctx *ParseContext) (map[string]*openapi30models.RefParameter, error) {
	node := nodeGetValue(parent, "parameters")
	if node == nil || !nodeIsMapping(node) {
		return nil, nil
	}

	params := make(map[string]*openapi30models.RefParameter)
