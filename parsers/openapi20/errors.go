...
func toParseError(err error) openapi20models.ParseError {
	var pe *ParseError
	if errors.As(err, &pe) {
		return openapi20models.ParseError{Message: pe.Message, Path: pe.Path, Kind: "error"}
	}
	return openapi20models.ParseError{Message: err.Error(), Kind: "error"}
}
...
func unknownFieldParseErrors(fields []UnknownField) []openapi20models.ParseError {
	if len(fields) == 0 {
		return nil
	}
	result := make([]openapi20models.ParseError, len(fields))
	for i, f := range fields {
		result[i] = openapi20models.ParseError{
