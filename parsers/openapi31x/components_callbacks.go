...
func parseComponentsCallbacks(parent *yaml.Node, ctx *ParseContext) (map[string]*openapi31models.CallbackRef, error) {
	node := nodeGetValue(parent, "callbacks")
	if node == nil || !nodeIsMapping(node) {
		return nil, nil
	}

	callbacks := make(map[string]*openapi31models.CallbackRef)
