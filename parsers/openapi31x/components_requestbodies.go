...
func parseComponentsRequestBodies(parent *yaml.Node, ctx *ParseContext) (map[string]*openapi31models.RequestBodyRef, error) {
	node := nodeGetValue(parent, "requestBodies")
	if node == nil || !nodeIsMapping(node) {
		return nil, nil
	}

	requestBodies := make(map[string]*openapi31models.RequestBodyRef)
